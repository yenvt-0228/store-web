...
import { toPageWindow } from '../common/utils/pagination.util';
...
/**
 * Everything `OrderGrpcController` would otherwise do itself.
 *
 * The controller is left with the transport concerns — which rpc, which guard,
 * which filter — and the work of turning a request into a domain call and a
 * domain result into a proto message lives here, where it can be tested without
 * a server and reused if a second transport ever wants the same mapping.
 */
@Injectable()
export class OrderGrpcService {
   ...
   * @returns The order as the proto declares it.
   */
   ...
   * @returns One page of that customer's orders, plus the unpaged total.
   */
  async listOrders(request: ListOrdersRequest): Promise<ListOrdersResponse> {
    ...
    // so the defaults are resolved here rather than declared in the message.
    const result = await this.orders.findAll(request.userId, {
      ...toPageWindow(request.page, request.limit),
